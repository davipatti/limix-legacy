...
class CGPbase_test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.n_samples = 100
        self.setXy()
        #self.X = NP.random.rand(self.n_samples,self.n_dimensions)
        covar  = dlimix_legacy.CCovSqexpARD(self.n_dimensions)
        ll  = dlimix_legacy.CLikNormalIso()
        covar_params = NP.array([1,1,1])
        lik_params   = NP.array([0.5])
        ...
        self.constrainU['covar'] = +10*NP.ones_like(covar_params);
        self.constrainL['covar'] = 0*NP.ones_like(covar_params);
        self.constrainU['lik'] = +5*NP.ones_like(lik_params);
        self.constrainL['lik'] = 0*NP.ones_like(lik_params);
    ...
    def test_fit(self):
        ...
        #run
        self.gpopt.opt()
        params = NP.concatenate((self.gp.getParams()['covar'],self.gp.getParams()['lik']))[:,0]
        params_true = NP.array([0.28822188,  0.35271548,  0.13709146,  0.49447424])
        RV = ((params-params_true)**2).max()<1e-6
        RV = RV & (NP.absolute(self.gp.LMLgrad()['lik']).max()<1E-4)
        RV = RV & (NP.absolute(self.gp.LMLgrad()['covar']).max()<1E-4)
